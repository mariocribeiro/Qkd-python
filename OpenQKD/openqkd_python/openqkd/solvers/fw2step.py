...
def frank_wolfe_step(grad: np.ndarray,
    ...
    """
    Resolve o subproblema linear do Frank-Wolfe:
        min  <grad, sigma>
        s.t. sigma in S  (restrições do protocolo)
    """
    ...
    if prob.status not in ["optimal", "optimal_inaccurate"]:
        raise RuntimeError(f"FW step SDP falhou: status={prob.status}")
...
def line_search(rho: np.ndarray,
    ...
    """
    Busca de linha exata em gamma ∈ [0, 1]:
        gamma* = argmin f(rho + gamma * direction)
    """
    gammas = np.linspace(0, 1, n_points)
    f_vals = []
    for g in gammas:
        rho_trial = enforce_psd(rho + g * direction)
        Grho      = apply_channel(kraus_ops, rho_trial)
        ZGrho     = apply_dephasing(Grho, key_dim, key_proj)
        f_vals.append(quantum_relative_entropy(Grho, ZGrho))
    return float(gammas[np.argmin(f_vals)])


# ── Loop principal de Frank-Wolfe ─────────────────────────────────────────────

def frank_wolfe_loop(rho0: np.ndarray,
                     kraus_ops: List[np.ndarray],
                     constraints_fn: Callable,
                     key_dim: int,
                     key_proj: Optional[List[np.ndarray]] = None,
    ...
    """
    Loop de Frank-Wolfe até convergência pelo gap primal-dual.
    Retorna (rho*, fw_gap) no ponto convergido.
    """
    ...
    for k in range(max_iter):
        ...
        # gap de Frank-Wolfe: f(rho) - <grad, sigma>
        Grho   = apply_channel(kraus_ops, rho)
        ZGrho  = apply_dephasing(Grho, key_dim, key_proj)
...
def dual_certificate_winick(rho_star: np.ndarray,
    ...
    rho_star   = np.array(rho_star.real,  dtype=np.float64)
    kraus_ops  = [np.array(K.real,        dtype=np.float64) for K in kraus_ops]
    Gamma      = [np.array(G.real,        dtype=np.float64) for G in Gamma]
    ...
    if epsilon is None:
        epsilon = min(1e-4, 1.0 / (np.e * (d_out - 1))) if d_out > 1 else 1e-4
    ...
    y   = cp.Variable(n)
    lhs = sum(y[i] * Gamma[i].T for i in range(n))
    ...
    zeta = (2 * epsilon * (d_out - 1) * np.log(d_out / (epsilon * (d_out - 1)))
            if d_out > 1 else 0.0)
...
def fw2step_solver(rho0: np.ndarray,
    ...
    rho_star, fw_gap = frank_wolfe_loop(
