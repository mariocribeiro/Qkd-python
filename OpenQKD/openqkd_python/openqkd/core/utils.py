...
def partial_trace(rho: np.ndarray, dims: List[int], axis: int = 0) -> np.ndarray:
    """
    ...
    # contrai os índices do subsistema `axis`
    indices = list(range(2 * n))
    i, j = axis, axis + n
    result = np.trace(rho_tensor, axis1=i, axis2=j)
    # reshape de volta para matriz
